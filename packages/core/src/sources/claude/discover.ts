/**
 * Claude Code session discovery: scan ~/.claude/projects/ for .jsonl session
 * files and build cheap picker metadata without a full parse.
 */
...
/**
 * Every Claude Code session a reference matches — an exact id, or a prefix of
 * one. Exact matches win outright: a full id that is also the prefix of a
 * longer one is not ambiguous.
 */
export async function resolveClaudeRef(ref: string): Promise<SessionRef[]> {
  ...
  for (const dirName of await listProjectDirs(projectsDir)) {
    ...
    for (const fileName of await listJsonlFiles(dirPath)) {
      ...
      if (sessionId === trimmed) exact.push(path.join(dirPath, fileName));
      else if (sessionId.startsWith(trimmed)) prefix.push(path.join(dirPath, fileName));
    }
  }
...
/** True when this file looks like a Claude Code session rather than another agent's. */
export function sniffClaude(firstLine: string): boolean {
  try {
    const parsed: unknown = JSON.parse(firstLine);
    if (typeof parsed !== 'object' || parsed === null) return false;
    const type = (parsed as Record<string, unknown>)['type'];
    return type === 'summary' || type === 'user' || type === 'assistant' || type === 'permission-mode';
  } catch {
    return false;
  }
}
